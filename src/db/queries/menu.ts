...
export async function createCategory(name: string) {
  ...
  const [row] = await db.insert(categories).values({ name, sortOrder: existing.length }).returning();
  return row;
}
...
export async function deleteProduct(id: number) {
  const groups = await db.select().from(modifierGroups).where(eq(modifierGroups.productId, id));
  for (const group of groups) {
    await db.delete(modifierOptions).where(eq(modifierOptions.modifierGroupId, group.id));
  }
  await db.delete(modifierGroups).where(eq(modifierGroups.productId, id));
...
export async function createModifierGroup(input: {
  ...
  const existing = await db.select().from(modifierGroups).where(eq(modifierGroups.productId, input.productId));
  const [row] = await db
...
export async function updateModifierGroup(
  ...
  await db.update(modifierGroups).set(input).where(eq(modifierGroups.id, id));
}
...
export async function createModifierOption(input: {
  ...
  const existing = await db
    .select()
    .from(modifierOptions)
    .where(eq(modifierOptions.modifierGroupId, input.modifierGroupId));
  const [row] = await db
...
export async function updateModifierOption(
  ...
  await db.update(modifierOptions).set(input).where(eq(modifierOptions.id, id));
}
